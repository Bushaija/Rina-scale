...
export function PlanActivityRow({ activity, index, form, isSubCategory, isReadOnly = false }: PlanActivityRowProps) {
  ...
  const frequency = shouldBeReadOnly ? activity.frequency : watch(`activities.${index}.frequency`);
  const unitCost = shouldBeReadOnly ? activity.unitCost : watch(`activities.${index}.unitCost`);
  const countQ1 = shouldBeReadOnly ? activity.countQ1 : watch(`activities.${index}.countQ1`);
  const countQ2 = shouldBeReadOnly ? activity.countQ2 : watch(`activities.${index}.countQ2`);
  const countQ3 = shouldBeReadOnly ? activity.countQ3 : watch(`activities.${index}.countQ3`);
  const countQ4 = shouldBeReadOnly ? activity.countQ4 : watch(`activities.${index}.countQ4`);
  const comment = shouldBeReadOnly ? activity.comment : watch(`activities.${index}.comment`);
  ...
  const amountQ1 = shouldBeReadOnly ? (activity.amountQ1 || 0) : (watch(`activities.${index}.amountQ1`) || 0);
  const amountQ2 = shouldBeReadOnly ? (activity.amountQ2 || 0) : (watch(`activities.${index}.amountQ2`) || 0);
  const amountQ3 = shouldBeReadOnly ? (activity.amountQ3 || 0) : (watch(`activities.${index}.amountQ3`) || 0);
  const amountQ4 = shouldBeReadOnly ? (activity.amountQ4 || 0) : (watch(`activities.${index}.amountQ4`) || 0);
  const totalBudget = shouldBeReadOnly ? (activity.totalBudget || 0) : (watch(`activities.${index}.totalBudget`) || 0);
  ...
  return (
    <TableRow className={isSubCategory ? "bg-muted/20" : "bg-muted/50 font-semibold"}>
      ...
      <TableCell className="w-[90px] text-center">{formatCurrency(amountQ1)}</TableCell>
      <TableCell className="w-[90px] text-center">{formatCurrency(amountQ2)}</TableCell>
      <TableCell className="w-[90px] text-center">{formatCurrency(amountQ3)}</TableCell>
      <TableCell className="w-[90px] text-center">{formatCurrency(amountQ4)}</TableCell>
